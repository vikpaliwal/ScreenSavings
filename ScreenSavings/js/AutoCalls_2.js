function checkData() {
    ...
    mssql.query(query, {
        ...
        },error:function(e)
        {
            ...
            console.log('Sorry couldnt get data');
        }
    });
		...
		var callback = function(response) {
		 ...
		 response.on('data', function (chunk) {
		  ...
		  response.on('end', function () {
              //console.log('No duplicate rows found.'+dataStatus);
              
			try
			{
				var dataStatus=JSON.parse(str);
                console.log('No duplicate rows found. yahoo'+Account.AccountID);
				if(dataStatus.isDataDifferent)
				{
					 push.wns.sendRaw(Account.ChannelURI,"Download",{
